...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            ...
            if (editable.Value)
            {
                groupUiObject = InformationModel.MakeObject<Panel>(group.BrowseName, CorsoFTOptix_VR_17_06_2025.ObjectTypes.GroupCheckbox);
            }
            else if (UserHasGroup(group.NodeId))
            {
                groupUiObject = InformationModel.MakeObject<Panel>(group.BrowseName, CorsoFTOptix_VR_17_06_2025.ObjectTypes.GroupLabel);
            }
